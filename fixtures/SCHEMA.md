...
Two file kinds, both `<plugin>.json` keyed by plugin slug (for example,
`custom-json`, `custom-form`, `custom-xml`, or `apprise-api`):
...
no normalisation (normalisation is the responsibility of `test/golden.ts`).
  ...
  "cases": [
    {
      ...
      "expectedCount": 2,            // optional hand-authored request-count oracle
...
The generic capture/replay runners validate `entropyHex` without installing it
for unrelated plugins or altering execution order. Only active Pushover E2EE
capture cases and their explicit sequential replay tests install the queue.
...
Multi-request cases SHOULD declare source `expectedCount`. Capture checks this
hand-authored value before writing a fixture, preventing a malformed canned
response from producing a self-consistent but truncated request chain. Generated
multi-request fixtures store the observed count as `expected.expectedCount`.
...
- **`requests`** `[{ method, url, headers, body }, ...]` + **`expectedCount`** —
  ...
  Emitted automatically when a case produces >1 request. The source case's
  hand-authored `expectedCount` is the independent capture oracle; the generated
  `expected.expectedCount` records the observed count (== `requests.length`).
  The TS diff asserts the plugin issues exactly that many requests, then performs
  the ordered field-by-field comparison.
